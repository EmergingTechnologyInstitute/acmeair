...
public class RESTCookieSessionFilter implements Filter {
	private static final Log log = LogFactory
			.getLog(RESTCookieSessionFilter.class);
	...
	private TransactionService getTxService() {
		if (!this.initializedTXService) {
			...
			transactionService = ServiceLocator
	...
	public void doFilter(ServletRequest req, ServletResponse resp,
		...
		try {
			...
			String path = request.getContextPath() + request.getServletPath()
					+ request.getPathInfo();
			// The following code is to ensure that OG is always set on the
			// thread
			try {
			...
			// could do .startsWith for now, but plan to move LOGOUT to its own
			// REST interface eventually
			...
			request.setAttribute(LOGIN_USER, "uid0@email.com");
			chain.doFilter(req, resp);
			return;
